...
export const Outfit = () => {
  ...
  return (
    <div style={{ width: '100%', height: '100%', paddingLeft: 74, paddingRight: 74, paddingTop: 36, paddingBottom: 36, background: '#CCD5AE', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: 7, display: 'flex' }}>
      {shoes && <div style={{ alignSelf: 'stretch', width: 257, height: 254, background: '#D9D9D9' }}>{shoes.image}</div>}
      {tops && <div style={{ alignSelf: 'stretch', width: 257, height: 254, background: '#D9D9D9' }}>{tops.image}</div>}
      {bottoms && <div style={{ alignSelf: 'stretch', width: 257, height: 254, background: '#D9D9D9' }}>{bottoms.image}</div>}
    </div>
  );
};
